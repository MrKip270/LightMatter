...
- **Two scores.** `score` answers "should I go out tonight?" (clouds + moon + light pollution). `potentialScore` answers "is this location worth a trip at all?" — what the site delivers on a clear, moonless night, so it depends only on light pollution and is stable across conditions. Both are produced by the **same** `scoreFrom(cloud, darkness, aurora)` function; the potential score is that function called with `cloud = 1` and no moon penalty. Implementing it as a second formula was rejected: the two definitions would drift and the comparison between them would stop meaning anything. The potential score deliberately **excludes the aurora bonus**, since aurora depends on an 11-year solar cycle and would make a "stable property of a place" change year to year.
...
- **Location handling:** The user enters a city/place or coordinates, uses device GPS, or **clicks a point on a map**; all four resolve to latitude/longitude, which is the key every data source is queried by.
- **Light pollution overlay: server-rendered PNG tiles, not a single image.** The grid is stored in EPSG:4326 (equal degrees per pixel); Leaflet renders in EPSG:3857 (Web Mercator). `L.imageOverlay` maps corner to corner *linearly in projected space*, so overlaying the raw grid would compress the tropics and squash high latitudes by hundreds of kilometres — precisely where the dark-sky sites are. Tiles avoid this by construction, since each tile's bounds are defined in Mercator and the projection is applied per pixel. `maxNativeZoom` is 8 because the grid is ~7 km per cell; beyond that Leaflet upscales, so the map goes visibly soft rather than implying detail the data lacks. Rendering is ~8 ms per tile after hoisting the per-row/per-column coordinate maths out of the inner loop (it was 78 ms when computed per pixel) plus a precomputed colour lookup table, with an LRU cache of encoded buffers.
- **Palette: the classic light-pollution rainbow, with a known flaw.** White → red → orange → yellow → green → blue → black, matching Falchi, Lorenz and lightpollutionmap.info so results are directly comparable against the reference maps. It is **not perceptually uniform** — yellow is about twice as luminant as the red preceding it, so the eye reads a boundary the data does not contain. Comparability was judged worth more than perceptual purity here; a test documents the limitation so it stays a decision rather than an oversight, and viridis is a one-function swap if that trade ever looks wrong.
- **Map picker: Leaflet 1.9.4 + OpenStreetMap tiles.** No API key, no build step, ~40 KB, plain JS — the only option that fits the no-framework decision. Loaded lazily from CDN when the map is first opened, so the common path (type a city, press enter) is unaffected and mobile visitors who never open it download nothing.
- **Reverse geocoding must be server-side.** Nominatim's usage policy requires a `User-Agent` header identifying the application, and browsers forbid JavaScript from setting `User-Agent` (it is a forbidden header name in the fetch spec). A compliant request is therefore *impossible* from the frontend — this is an architectural requirement, not a preference. `backend/sources/reversegeocode.js` enforces the 1 request/second limit and caches results, because the frontend is not the only possible caller of our API. **BigDataCloud was rejected** despite a keyless, unlimited free tier: its fair use policy covers only the *current* coordinates of the client device obtained via the platform's location API, and a map click is a third-party coordinate.
- **Naming failures degrade to coordinates.** `/api/reverse-geocode` always answers `200`; an outage or an unnameable location (open ocean, wilderness) returns `label: null`. The sky report needs only coordinates, so a cosmetic lookup must never be able to break it.
